"""Test client for CSP MCP server."""
...
async def main():
    ...
    print("=" * 60)
    print(f"CSP MCP Server Test Client")
    ...
    async with streamablehttp_client(server_url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            # Note: steps_getAverageDuration is now internal-only (not exposed as MCP tool)
            # It's called automatically during plan.create and plan.update

            # Test 1: Log a completed step (legacy)
            print("\n1. Testing steps_logCompleted (legacy)...")
            try:
